    ...
    Object.keys(users).forEach(user => {
        describe(`${user} users`, () => {
            it.skip('create new record', async () => {
                ...
                const response = await mockRequest.post('/api/v2/music').send({
                    musicName: "test",
                    releaseDate: "test"
            ...
            it.skip('get all records', async () => {
                ...
                const response = await mockRequest.get('/api/v2/music').set('Authorization', `Bearer ${token}`)
            ...
            it.skip('get one record', async () => {
                ...
                const token = register.body.token;
                const response = await mockRequest.get(`/api/v2/music/${id}`).set('Authorization', `Bearer ${token}`);
            ...
            it.skip('update record', async () => {
                ...
                const response = await mockRequest.put(`/api/v2/music/${id}`).send({
                    name: "test1",
                    duration: 30
                }).set("Authorization", `Bearer ${token}`);
            ...
            it.skip('delete record', async () => {
                const register = await mockRequest.post('/signin').auth(users[user].username, users[user].password);
                const token = register.body.token;
                const response = await mockRequest.delete('/api/v2/music/1').set('Authorization', `Bearer ${token}`);
